...
- [x] 2.5 CountdownTarget（单目标，user 唯一约束）
...
- [ ] 15.1 新增 Feedback 模型（type = vote/request、title、description、vote_count、时间戳）
- [ ] 15.2 新增 Vote 模型（user_id + feedback_id 唯一约束，实现"每用户每功能限投 1 票"）
- [ ] 15.3 创建 routers/feedback.py 蓝图并在 app.py 注册
- [ ] 15.4 POST /api/feedback/vote（为功能投票，重复投票返回"您已为该功能投票"，vote_count +1）
- [ ] 15.5 POST /api/feedback/request（提交功能需求，1-200 字校验，无需审核提交即入库）
- [ ] 15.6 GET /api/feedback/ranking（反馈排行榜：功能受欢迎排行 + 用户需求列表）
- [ ] 15.7 所有接口需登录、按 user_id 隔离；预设可投票功能列表初始化

### 任务 16：实现前端用户中心页 — 承载 F-04 / F-05 / F-06 / F-33 入口
- [ ] 16.1 新增"我的"导航项与 UserPage 渲染/初始化函数
- [ ] 16.2 用户信息区（邮箱显示、has_password 状态）
- [ ] 16.3 设置密码入口（调用 /api/auth/set-password；无密码账号提示设置，有密码账号提供修改）
- [ ] 16.4 注销账户入口（二次确认，调用 /api/auth/account DELETE，清除本地态）
- [ ] 16.5 退出登录入口（调用 /api/auth/logout，清除 token）
- [ ] 16.6 常用工具排行区（复用 getTop，展示 Top 列表 + 使用次数）
- [ ] 16.7 我的反馈入口（跳转/进入反馈页面）

### 任务 17：实现前端反馈页面 — F-34 / F-35 / F-36 UI
- [ ] 17.1 功能投票区（预设功能列表 + 投票按钮 + 已投状态标记）
- [ ] 17.2 提交需求表单（标题/描述，1-200 字前端校验）
- [ ] 17.3 反馈排行榜（功能票数排行 + 用户需求列表，提交即展示）
- [ ] 17.4 封装 feedbackApi（vote / request / ranking）
- [ ] 17.5 游客访问时弹出登录引导

### 任务 18：倒计时支持多目标 — F-18（PRD P1，当前仅单目标）
- [ ] 18.1 后端 CountdownTarget 放开 user 唯一约束，改为列表模式
- [ ] 18.2 后端新增 GET 列表 / POST 新增 / PUT 编辑 / DELETE 删除单条
- [ ] 18.3 前端倒计时页改为列表展示 + 新增/编辑/删除
- [ ] 18.4 兼容旧的单目标数据迁移

### 任务 19：全局联调与边界处理
- [ ] 19.1 联调反馈模块（投票 / 提交需求 / 排行榜）端到端
- [ ] 19.2 联调用户中心（设置密码 / 注销账户 / 退出登录 / 常用排行）
- [ ] 19.3 验证反馈输入边界（1-200 字、重复投票提示）
- [ ] 19.4 验证数据隔离（反馈/统计按 user_id，禁止越权）
- [ ] 19.5 验证游客拦截（反馈/用户中心需登录，弹出登录引导）
- [ ] 19.6 验证注销后反馈/统计/个人数据物理删除
- [ ] 19.7 天气 API 失败时展示最近缓存结果（当前仅提示重试）
...
- 任务 18（多倒计时）为 PRD P1 但非本次重点，可排在反馈模块之后。
